<template>
  <div class="chat-window">
    ...
    <div class="top">
      ...
      <div class="other-fun">
        <span class="iconfont icon-shezhi" @click="Return"> </span> <!-- 视频按钮 -->
    ...
    <div class="botoom">
      ...
      <div class="chatInputs">
        ...
        <div class="send boxinput" @click="analyiseText">
...
export default {
      ...
      headImages: [
        {
          ...
          img:"https://pic.imgdb.cn/item/6711d0bad29ded1a8c1c99c6.png"
        },
        {
          name: "Albert Einstein",
          img:"https://pic.imgdb.cn/item/6711d0bad29ded1a8c1c9989.jpg"
        },
        {
          name: "Marie Curie",
          img:"https://pic.imgdb.cn/item/6711d0bad29ded1a8c1c9a49.jpg"
        },
      ]
    };
  },
    ...
    async updateSet(updateSelections) {
      ...
      /*try {
        ...
        const res = await axios.post('http://localhost:5000/correct_writing', {
          ...
          modelContent: this.selections.modelContent,
          isNeedSynonyms: this.selections.selectedSynonym,
          correctVersion: this.selections.selectedCorrectVersion,
          isNeedEssayReport: this.selections.selectedEssayReport
        });
        // 将响应结果存储到 response 变量中
  ...
  try {
    const response = await api.post('http://localhost:5080/chat', message);
    for (let i = 0; i < response.data.output.length; i++) {
      let trText = await this.translateText(response.data.output[i].split(' : ')[1]);
      console.log(trText);
      let img="https://pic.imgdb.cn/item/6711af2ed29ded1a8cef4537.png"
      for(let j = 0 ; j < this.headImages.length; j++) {
        if(this.headImages[j].name==response.data.output[i].split(' : ')[0]){
          img=this.headImages[j].img;
          break;
        }
      }
      this.sendMsg({
        headImg: img,
        name: response.data.output[i].split(' : ')[0],
        time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
        msg: response.data.output[i].split(' : ')[1],
        translateText: trText,
        showTranslation: false,
        chatType: 0, // 信息类型，0代表文字
        uid: "1002", // 发送者 ID
